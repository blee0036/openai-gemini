// 常量
const ONDEMAND_API_BASE_URL = 'https://agentforge-api.aitech.io'; // API基础URL，根据要求硬编码
...
const __query_prefix = '尊重我的需求，用中文回答我'; // OnDemand查询的固定中文前缀
      ...
      if (item.type === 'image_url' && item.image_url && item.image_url.url) {
        ...
        try {
          if (imageUrl.startsWith('data:image')) {
            // 如果是Base64编码的图像数据
            // 检查R2配置是否完整
            if (env.R2_BUCKET && getEnv(env, 'R2_PUBLIC_URL_PREFIX')) {
              imageUrl = await uploadBase64ToR2(imageUrl, env); // 上传到R2
              console.log(`[+] 图像通过R2成功上传, URL: ${imageUrl}`);
            } else {
              console.warn('[!] R2未完全配置。无法上传Base64图像。如果媒体处理需要公共URL，则可能会失败。');
            }
          }
          // 调用媒体上传接口处理图像URL (可能是原始URL或R2 URL)
          mediaExtractedText = await mediaUpload(sessionId, imageUrl, onDemandApiKey, env);
          console.log(`[+] 图像内容已提取: ${mediaExtractedText}`);
        } catch (e) {
          console.error(`处理图像失败: ${e.message}`, e.stack);
          mediaExtractedText = '错误：处理图像失败。';
        }
        break; // 假设每个消息只处理一个图像
      }
    }
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/v1/chat/completions' && request.method === 'POST') {
      try {
        ...
        let sessionIdentifier = 'anonymous'; // 默认会话标识符
        ...
        if (openAIRequest.stream) {
          ...
          if (!onDemandResponse.ok) {
            ...
            throw new Error(`OnDemand查询失败: ${onDemandResponse.status} ${errorBody}`);
          }
